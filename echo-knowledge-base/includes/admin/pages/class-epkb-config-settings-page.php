...
class EPKB_Config_Settings_Page {
	...
	private $is_frontend_editor;
	private $elay_enabled;
	private $asea_enabled;
	private $ai_search_enabled;
	...
	public function __construct( $kb_config, $is_frontend_editor = false ) {
		...
		$this->kb_config_specs = EPKB_Core_Utilities::retrieve_all_kb_specs_with_labels( $this->kb_config['id'] );
		...
		$this->ai_search_enabled = EPKB_AI_Utilities::is_ai_search_enabled();
		...
		$this->is_basic_layout = EPKB_Layout::BASIC_LAYOUT == $this->kb_config['kb_main_page_layout'] && ! $this->kb_main_page_has_kb_blocks;
		$this->is_tabs_layout = EPKB_Layout::TABS_LAYOUT == $this->kb_config['kb_main_page_layout'] && ! $this->kb_main_page_has_kb_blocks;
		$this->is_categories_layout = EPKB_Layout::CATEGORIES_LAYOUT == $this->kb_config['kb_main_page_layout'] && ! $this->kb_main_page_has_kb_blocks;
		$this->is_classic_layout = EPKB_Layout::CLASSIC_LAYOUT == $this->kb_config['kb_main_page_layout'] && ! $this->kb_main_page_has_kb_blocks;
		$this->is_drill_down_layout = EPKB_Layout::DRILL_DOWN_LAYOUT == $this->kb_config['kb_main_page_layout'] && ! $this->kb_main_page_has_kb_blocks;
		$this->is_grid_layout = EPKB_Layout::GRID_LAYOUT == $this->kb_config['kb_main_page_layout'] && ! $this->kb_main_page_has_kb_blocks;
		$this->is_elay_layout = EPKB_Layouts_Setup::is_elay_layout( $this->kb_config['kb_main_page_layout'] ) && ! $this->kb_main_page_has_kb_blocks;
		$this->is_sidebar_layout = EPKB_Layout::SIDEBAR_LAYOUT == $this->kb_config['kb_main_page_layout'] && ! $this->kb_main_page_has_kb_blocks;
	...
	public function get_vertical_tabs_config() {
		...
		} else if ( $this->kb_main_page_has_kb_blocks ) {
			$main_page_contents = array_merge(
				array(
					array(
						'title_before_icon' => false,
		...
		$shortcode_main_page_sub_tabs = array();
		if ( $this->is_frontend_editor ) {
			$shortcode_main_page_sub_tabs = array(
				array(
					'title'     =>  esc_html__( 'Page Settings', 'echo-knowledge-base' ),
					'key'       => 'main-page-settings',
					'data'		=> ['selected-module' => 'main-page-settings', 'non-row-feature' => 'on'],
				)
			);
		}
		$shortcode_main_page_sub_tabs = array_merge( $shortcode_main_page_sub_tabs, array(
			array(
				'icon'      => 'epkb-admin__form-sub-tab-icon-text',
				'icon_text' => esc_html__( 'Row 1', 'echo-knowledge-base' ),
				'title'     =>  $row_1_module == 'none' ? $no_module_label : $this->kb_config_specs['ml_row_1_module']['options'][$row_1_module],
			...
			array(
				...
				'title'     => $row_2_module == 'none' ? $no_module_label : $this->kb_config_specs['ml_row_2_module']['options'][$row_2_module],
			...
			array(
				...
				'title'     => $row_3_module == 'none' ? $no_module_label : $this->kb_config_specs['ml_row_3_module']['options'][$row_3_module],
			...
			array(
				...
				'title'     => $row_4_module == 'none' ? $no_module_label : $this->kb_config_specs['ml_row_4_module']['options'][$row_4_module],
			...
			array(
				...
				'title'     => $row_5_module == 'none' ? $no_module_label : $this->kb_config_specs['ml_row_5_module']['options'][$row_5_module],
		...
		$tabs_config['archive-page'] = array(
			'title'  => esc_html__( 'Category Page', 'echo-knowledge-base' ),
	...
	private function show_kb_setting_html( $setting_name, $show_included=false ) {
		...
		if ( in_array( $setting_name, [ 'toc_toggler', 'toc_locations', 'toc_left', 'toc_content', 'toc_right', 'advanced_search_mp_presets', 'advanced_search_ap_presets', 'archive_content_sub_categories_display_mode',
			...
			'general_typography', 'sidebar_main_page_intro_text_link', 'article_search_sidebar_layout_msg', 'article_list_spacing', 'ml_categories_articles_sidebar_desktop_width',
			'archive_header_desktop_width', 'archive_content_desktop_width', 'article-container-desktop-width-v2', 'article-body-desktop-width-v2', 'article_sidebar_settings_link', 'categories_articles_preset',
			'template_main_page_display_title_info'] ) ) {
	...
	private function show_custom_display_fields( $setting_name, $show_included=false ) {
		...
		// not real setting; used by settings UI to control TOC left/middle/right visibility
			...
			<div class="epkb-input-group epkb-admin__radio-icons <?php echo esc_attr( $input_group_class ); ?>" id="<?php echo esc_attr( $setting_name ); ?>_group" <?php echo $group_data_escaped; ?>>
				...
				<div class="epkb-radio-buttons-container" id="<?php echo esc_attr( $setting_name ); ?>">              <?php
					...
					foreach( $this->kb_config_specs[$setting_name]['options'] as $key => $label ) {
						if ( $this->is_frontend_editor && !$this->elay_enabled && ( $key == 'Grid' || $key == 'Sidebar' ) ) {
							continue;
						}   ?>
					...
					if ( ! $this->elay_enabled ) {
						if ( $this->is_frontend_editor ) {
							echo 'Unlock Grid and Sidebar Layout By Upgrading to PRO';	// TODO: add this to the frontend editor
						} else {
							...
							'title' => sprintf( esc_html__( "Unlock %1\$sGrid and Sidebar Layout%2\$s By Upgrading to PRO", 'echo-knowledge-base' ), '<strong>', '</strong>' ),
			...
			if ( ! $this->is_frontend_editor ) {
				...
				<div class="epkb-admin__input-field <?php echo esc_attr( $input_group_class ); ?>" <?php echo $group_data_escaped; ?>>  <?php
					...
					EPKB_HTML_Forms::pro_feature_ad_box( array(
						'title'      	=> sprintf( esc_html__( "Get %1\$sArticle Ratings and Feedback%2\$s Feature", 'echo-knowledge-base' ), '<strong>', '</strong>' ),
						'list'       	=> array(
		...
		if ( in_array( $setting_name, [ 'asea_pro_description' ] ) ) {
			...
			<div class="epkb-admin__input-field <?php echo esc_attr( $input_group_class ); ?>" <?php echo $group_data_escaped; ?>>  <?php
				...
				// translators: %1$s and %2$s are HTML strong tags
				EPKB_HTML_Forms::pro_feature_ad_box( array(
					'title'        	=> sprintf( esc_html__( "Upgrade To %1\$sAdvanced Search%2\$s", 'echo-knowledge-base' ), '<strong>', '</strong>' ),
					'list'       	=> array(
		...
		if ( $setting_name == 'elay_pro_description' ) {
			if ( $this->is_frontend_editor ) {
				echo 'Get Elegant Layouts Feature By Upgrading to PRO';	// TODO: add this to the frontend editor
			} else {
				$group_data_escaped = EPKB_HTML_Elements::get_data_escaped( $group_data );  ?>
				<div class="epkb-admin__input-field <?php echo esc_attr( $input_group_class ); ?>" <?php echo $group_data_escaped; ?>>  <?php
					//phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
				// translators: %1$s and %2$s are HTML strong tags
				EPKB_HTML_Forms::pro_feature_ad_box( array(
					'title'      	=> sprintf( esc_html__( "Upgrade To %1\$sElegant Layouts%2\$s", 'echo-knowledge-base' ), '<strong>', '</strong>' ),
					'list'       	=> array(
		...
		if ( $setting_name == 'general_typography' ) {
			...
			if ( ! empty( $input_args['setting_help_text'] ) ) {
				EPKB_HTML_Elements::display_input_bottom_external_links( $input_args['setting_help_text'], ['css_class' => 'epkb-general-typography-description'] );
			}
		}

		if ( $setting_name == 'sidebar_main_page_intro_text_link' ) {
			$url = admin_url( 'edit.php?post_type=' . EPKB_KB_Handler::get_post_type( $this->kb_config['id'] ) . '&page=epkb-kb-configuration#settings__labels____sidebar_main_page_intro_text' ); ?>
		...
		if ( $setting_name == 'article_search_sidebar_layout_msg' ) {
			...
			<div class="epkb-admin__input-field">
				<p><?php
					...
					echo sprintf( esc_html__( 'You have chosen the Sidebar layout. The Sidebar layout search box is controlled by the %1$ssearch settings on the Main Page%2$s.', 'echo-knowledge-base' ), '<a class="epkb-admin__form-tab-content-desc__link" href="' . esc_url( $main_page_url ) . '" target="_blank" rel="noopener noreferrer">', '</a>' ); ?>
				</p>
			</div>  <?php
		}
		...
		if ( $setting_name == 'article_sidebar_settings_link' ) {
			if ( $this->is_frontend_editor ) {
				$input_args = $this->set_input_tooltip( [ 'specs' => 'elay_sidebar_article_icon', ] );
				EPKB_HTML_Elements::radio_buttons_icon_selection( array_merge_recursive( $input_args, [
					'value'             => $this->kb_config['elay_sidebar_article_icon'],
					'input_group_class' => $input_group_class,
				] ) );
			} else {
				...
				<div class="epkb-admin__input-field ">
					<p>
						<?php esc_html_e( 'To Edit Featured Articles Settings', 'echo-knowledge-base' ); ?>
						<a class="epkb-admin__form-tab-content-desc__link epkb-admin__form-tab-content-desc__link--list-of-articles" href="<?php echo esc_url( $url ); ?>"><?php esc_html_e( 'Click here', 'echo-knowledge-base' ); ?></a>
					</p>
				</div>  <?php
			}
		}

		// Page Title info message for Current Theme Template
		if ( $setting_name == 'template_main_page_display_title_info' ) {
			$group_data_escaped = EPKB_HTML_Elements::get_data_escaped( $group_data );  ?>
			<div class="epkb-admin__input-field <?php echo esc_attr( $input_group_class ); ?>" id="<?php echo esc_attr( $setting_name ); ?>" <?php echo $group_data_escaped; ?>>
				<?php EPKB_HTML_Forms::notification_box_middle( [
					'type' => 'info',
					'desc' => esc_html__( 'The page title is controlled by your theme.', 'echo-knowledge-base' ),
				] ); ?>
			</div>  <?php
		}
	...
	private function get_quick_links_box() {
		...
		if ( current_user_can( EPKB_Admin_UI_Access::get_context_required_capability( 'admin_eckb_access_frontend_editor_write' ) ) ) {   ?>
			<div class="epkb-kb__btn-wrap"> <?php
				// if user already ran Setup Wizard, then show link to FE Editor
	...
	private function get_contents_configs() {
		...
		$contents_configs['archive-page'][] = array(
			...
			'fields'    => [
				...
				'section_head_category_icon_color'              => [ 'only_archive_kb_templates', 'only_archive_page_v3', 'only_block_main_page' ],
				'section_head_font_color'                       => [ 'only_archive_kb_templates', 'only_archive_page_v3', 'only_block_main_page' ],
			...
			/* FUTURE TODO as info icon 'learn_more_links' => [ // title => url
				__( 'Category Page', 'echo-knowledge-base' ) => 'https://www.echoknowledgebase.com/documentation/category-archive-page/',
		...
		$contents_configs['other'] = [
			array(
				...
				'fields'    => [
					...
					'template_main_page_display_title_info' => 'not_kb_templates',
				],
			),
			array(
			...
			array(
				'title'     => $this->kb_main_page_has_kb_blocks ? esc_html__( 'Typography for Article Page and Category Archive Page', 'echo-knowledge-base' ) : esc_html__( 'Typography', 'echo-knowledge-base' ),
				'desc'      => '',
				'fields'    => [
					'general_typography' => '',
			...
			array(
				...
				'fields'            => [
					'epkb_ml_custom_css' => [ 'not_block_main_page' ],
				],
			),
		];
	...
	private function get_sub_contents_configs() {
		...
		$sub_contents_configs = array_merge_recursive( $sub_contents_configs, array(
			...
			'article-page-settings' => [
				...
				array(
					...
					'data'      => [ 'target' => 'article_template_spacing' ],
				),
				array(
		...
		return $this->add_sub_contents_configs( $sub_contents_configs );
	}
	...
	private function set_custom_field_specs( $field_specs ) {
		...
		switch ( $field_specs['name'] ) {
			...
			case 'advanced_search_ap_input_box_radius':
			case 'advanced_search_mp_input_box_radius':
				if ( $this->is_frontend_editor ) {
					$field_specs['label'] = esc_html__( 'Search Input Radius', 'echo-knowledge-base' );
				}
				break;
		...
		$field_specs = $this->set_all_custom_field_specs( $field_specs );
	...
	private function set_input_tooltip( $input_args ) {
		...
		switch ( $input_args['specs'] ) {
			...
			case 'ml_categories_articles_sidebar_desktop_width':
				$input_args['tooltip_body'] =  esc_html__( 'The width of the sidebar is determined by the Row Width setting. If the Row Width is set to 100%, the Sidebar Width can be assigned, for example, as 30% (Large), leaving the remaining 70% for the rest of the row.', 'echo-knowledge-base' ) . '<br/><br/>' .
...
//				break;
			...
			case 'ml_faqs_title_location':
				// translators: %1$s and %2$s are HTML anchor tags
				$input_args['tooltip_body'] =  sprintf( esc_html__( 'To change FAQ Title %1$sclick here%2$s', 'echo-knowledge-base' ), '<a href="#" class="epkb-admin__form-tab-content-desc__link">', '</a>' );
				break;

			case 'ml_articles_list_title_location':
				// translators: %1$s and %2$s are HTML anchor tags
				$input_args['tooltip_body'] =  sprintf( esc_html__( 'To change Featured Articles Title %1$sclick here%2$s', 'echo-knowledge-base' ), '<a href="#" class="epkb-admin__form-tab-content-desc__link">', '</a>' );
				break;
			...
			case 'template_for_archive_page':
				...
				if ( ! $this->is_frontend_editor && ! empty( $example_terms ) && is_array( $example_terms ) ) {
				...
				$input_args['setting_help_text'][] = [
					'help_desc' => esc_html__( 'The difference between KB Template and the Current Theme Template is explained in detail', 'echo-knowledge-base' ) . ':',
					'link_text' => esc_html__( 'Learn More', 'echo-knowledge-base' ),
					'link_url' => 'https://www.echoknowledgebase.com/documentation/category-archive-page/',
					'is_bottom_text'=> true ];
			...
			case 'templates_for_kb':
				...
				// scenario: Block theme with KB Block Template
				if ( EPKB_Block_Utilities::is_block_theme() && EPKB_Block_Utilities::is_kb_block_page_template_available() ) {

					$input_args['tooltip_body'] = esc_html__( 'Block Template to use for KB Main Page', 'echo-knowledge-base' );
					$input_args['setting_help_text'] = [];
					$kb_main_page_url = EPKB_KB_Handler::get_first_kb_main_page_url( $this->kb_config );
					if ( !$this->is_frontend_editor && !empty( $kb_main_page_url ) ) {
						$input_args['setting_help_text'][] = [
							'help_desc' => esc_html__( 'After you switch templates, see how it looks on the frontend', 'echo-knowledge-base' ),
							'link_text' => esc_html__( 'here', 'echo-knowledge-base' ),
					...
					$input_args['setting_help_text'][] = [
						'help_desc' => esc_html__( 'The difference between the KB Template and the Current Theme Template is explained in detail. This setting also applies to the Article Page.', 'echo-knowledge-base' ) . ':',
						'link_text' => esc_html__( 'Learn More', 'echo-knowledge-base' ),
						'link_url' => 'https://www.echoknowledgebase.com/documentation/kb-block-template/',
						'is_bottom_text' => true];
					if ( $this->is_frontend_editor ) {
						$input_args['setting_help_text'][] = [
							'help_desc' => esc_html__('The Category Archive Page has a separate setting', 'echo-knowledge-base') . ':',
							'link_text' => esc_html__('Learn More', 'echo-knowledge-base'),
							'link_url' => 'https://www.echoknowledgebase.com/documentation/category-archive-page/',
							'is_bottom_text' => true];
					}

				// scenario: WP < 6.7 or Classic non-block theme
				} else {
			...
			case 'section_head_category_icon_location':
				...
				$input_args['tooltip_body'] = sprintf(
					esc_html__(
						'Set Image and Font Icons for Categories. %1$sLearn More%2$s',
						'echo-knowledge-base'
					),
	...
	/**
	 * Add sub-contents configs for Main Page
	 *
	 * @param $sub_contents_configs
	 * @return mixed
	 */
	private function add_sub_contents_configs( $sub_contents_configs ) {
		...
		$modules_list = [
			...
			'search' => [
				...
				'main-page-search-box' => [
					...
					'advanced_search_mp_input_box_radius' => 'asea',
					'advanced_search_mp_input_box_search_icon_placement' => 'asea',
					'advanced_search_faqs_toggle' => 'asea',
					...
					'search_box_input_height' => 'not_asea',
			...
			'main-page-search-results-list' => [
				'advanced_search_mp_results_list_size' => [ 'asea', 'not_ai_search' ],
				'advanced_search_mp_show_top_category' => 'asea',
				'advanced_search_context_toggle' => 'asea',
				'advanced_search_context_highlight_font_color' => 'asea',
				'advanced_search_context_characters' => 'asea',
			],
			'main-page-search-button' => [
				'advanced_search_mp_ai_search_button_background_color' => 'asea',
				'advanced_search_mp_ai_search_button_text_color' => 'asea',
				'advanced_search_mp_ai_search_button_background_hover_color' => 'asea',
				'advanced_search_mp_ai_search_button_text_hover_color' => 'asea',
			],
			'main-page-search-labels-box' => [
					...
					'advanced_search_mp_title_font_size' => 'asea',
		...
		if ( $this->is_frontend_editor ) {
			$sub_contents_configs['main-page-settings'] = array(
				'main-page-page-title' => array(
					'title'     => __( 'Page Title', 'echo-knowledge-base' ),
					'desc'      => '',
					'css_class'	=> 'epkb-admin__form-tab-content--main-page-page-title epkb-admin__form-tab-content--module-box epkb-admin__form-tab-content--main-page-settings-box epkb-admin__form-tab-content--hide',
					'fields'    => [
						'template_main_page_display_title' => '',
						'template_main_page_display_title_info' => '',
					],
					'data'		=> [ 'target' => 'main-page-page-title' ],
				),
				'theme-compatibility-mode' => array(
					'title'     => __( 'Choose Template', 'echo-knowledge-base' ),
					'desc'      => '',
					'css_class'	=> 'epkb-admin__form-tab-content--theme-compatibility-mode epkb-admin__form-tab-content--module-box epkb-admin__form-tab-content--main-page-settings-box epkb-admin__form-tab-content--hide',
					'fields'    => [
						'templates_for_kb' => 'Choose Template',
					],
					'data'		=> [ 'target' => 'theme-compatibility-mode' ],
				),
				'main-page-template-spacing' => array(
					'title'     => __( 'KB Page Spacing', 'echo-knowledge-base' ),
					'desc'      => '',
					'css_class'	=> 'epkb-admin__form-tab-content--main-page-template-spacing epkb-admin__form-tab-content--module-box epkb-admin__form-tab-content--main-page-settings-box epkb-admin__form-tab-content--hide',
					'fields'    => [
						'template_main_page_padding_top' => '',
						'template_main_page_margin_top' => '',
					],
					'data'		=> [ 'target' => 'main-page-template-spacing' ],
				),
				'main-page-general-typography' => array(
					'title'     => __( 'Font Family', 'echo-knowledge-base' ),
					'desc'      => '',
					'css_class'	=> 'epkb-admin__form-tab-content--main-page-general-typography epkb-admin__form-tab-content--module-box epkb-admin__form-tab-content--main-page-settings-box epkb-admin__form-tab-content--hide',
					'fields'    => [
						'general_typography' => '',
					],
					'data'		=> [ 'target' => 'main-page-general-typography' ],
				),
				'main-page-custom-css' => array(
					'title'     => __( 'Custom CSS for Main Page', 'echo-knowledge-base' ),
					'desc'      => '',
					'css_class'	=> 'epkb-admin__form-tab-content--main-page-custom-css epkb-admin__form-tab-content--module-box epkb-admin__form-tab-content--main-page-settings-box epkb-admin__form-tab-content--hide',
					'fields'    => [
						'epkb_ml_custom_css' => '',
					],
					'data'		=> [ 'target' => 'main-page-custom-css' ],
				)
			);
		}

		// 1st Row
		$sub_contents_configs['main-page-ml-row-1']['module-selection'] = array(
		...
		if ( ! $this->is_frontend_editor ) {
			$sub_contents_configs['main-page-ml-row-1']['theme-compatibility-mode'] = array(
				'title'     => esc_html__( 'Template Setup', 'echo-knowledge-base' ),
				'desc'      => '',
		...
		$sub_contents_configs['main-page-ml-row-1']['layout'] = array(
		...
		$sub_contents_configs['main-page-ml-row-1']['module-settings'] = array(
			'title'     => esc_html__( 'Settings', 'echo-knowledge-base' ),
		...
		// 	Main Page version of Settings boxes for 'old' layouts: Basic, Tabs, Category, Grid, Sidebar
		$sub_contents_configs['main-page-ml-row-1'] = array_merge_recursive( $sub_contents_configs['main-page-ml-row-1'], array(
			...
			array(
				...
				'fields'        => [
					...
					'sidebar_section_border_width' => [ 'elay', 'only_sidebar' ],
					'sidebar_section_border_color' => [ 'elay', 'only_sidebar' ],
					'sidebar_section_border_radius' => [ 'elay', 'only_sidebar' ],
					'sidebar_section_box_shadow' => [ 'elay', 'only_sidebar' ],
			...
			array(
				...
				'fields'        => [
					...
					// Sidebar Layout
					'sidebar_section_head_alignment' => [ 'elay', 'only_sidebar' ],
					'sidebar_section_head_font_color' => [ 'elay', 'only_sidebar' ],
					'sidebar_section_head_background_color' => [ 'elay', 'only_sidebar' ],
					'sidebar_section_head_description_font_color' => [ 'elay', 'only_sidebar' ],
					'sidebar_section_divider' => [ 'elay', 'only_sidebar' ],
					'sidebar_section_divider_color' => [ 'elay', 'only_sidebar' ],
					'sidebar_top_categories_collapsed' => [ 'elay', 'only_sidebar' ],
					...
					'section_divider_thickness' => [ [ 'only_basic', 'only_tabs', 'only_categories' ] ],
			...
			array(
				...
				'fields'        => [
					...
					'grid_section_icon_padding_top' => [ 'elay', 'only_grid' ],
					'grid_section_icon_padding_bottom' => [ 'elay', 'only_grid' ],
					'grid_section_icon_padding_left' => [ 'elay', 'only_grid' ],
					'grid_section_icon_padding_right' => [ 'elay', 'only_grid' ],
					'sidebar_section_category_icon_color' => [ 'elay', 'only_sidebar' ],
					'sidebar_section_category_font_color' => [ 'elay', 'only_sidebar' ],
			...
			array(
				...
				'fields'        => [
					...
					'article_list_margin' => [ 'not_grid', 'not_sidebar', 'not_drill_down' ],
					'sub_article_list_margin' => [ 'not_categories', 'not_grid', 'not_sidebar', 'not_drill_down' ],
					...
					'sidebar_article_icon_toggle' => [ 'elay', 'only_sidebar' ],
					'article_font_color' => [ 'not_grid', 'not_sidebar' ],
					'sidebar_article_font_color' => [ 'elay', 'only_sidebar' ],
					'sidebar_article_active_font_color' => [ 'elay', 'only_sidebar' ],
					'sidebar_article_active_background_color' => [ 'elay', 'only_sidebar' ],
					'sidebar_show_articles_before_categories' => [ 'elay', 'only_sidebar' ],
	...
	// Main Page Search Button (AI Search) - only show if AI Search is enabled
	if ( $this->asea_enabled && EPKB_AI_Utilities::is_ai_search_smart_enabled() ) {
		$dedicated_module_boxes_config['main-page-search-button'] = [
			'module'    => 'search',
			'title'     => esc_html__( 'Search Button', 'echo-knowledge-base' ),
			'data'      => [ 'target' => 'search-button-mp' ],
		];
	}

	// Search Results List
		$dedicated_module_boxes_config['main-page-search-results-list'] = [
			'module'    => 'search',
			'title'     => esc_html__( 'Search Results List', 'echo-knowledge-base' ),
			'data'      => [ 'target' => 'search-results-list' ],
		];
		...
		if ( $this->is_frontend_editor || ( $this->kb_config['article_search_toggle'] == 'on' && ( $this->kb_config['article_search_sync_toggle'] != 'on' || $this->kb_main_page_has_kb_blocks ) ) ) {
			$sub_contents_configs['article-page-search-box'] = [
				...
				array(
					...
					'fields'    => [
						...
						'advanced_search_ap_input_box_radius' => 'asea',
						...
						'advanced_search_ap_results_list_size' => [ 'asea', 'not_ai_search' ],
			...
			array(
				'title'     => esc_html__( 'Search Button', 'echo-knowledge-base' ),
				'fields'    => [
					'advanced_search_ap_ai_search_button_background_color' => 'asea',
					'advanced_search_ap_ai_search_button_text_color' => 'asea',
					'advanced_search_ap_ai_search_button_background_hover_color' => 'asea',
					'advanced_search_ap_ai_search_button_text_hover_color' => 'asea',
				],
				'data'      => ['target' => 'search-button-ap'],
			),
			array(
				'title'     => esc_html__( 'Search Labels', 'echo-knowledge-base' ),
					'fields'    => [
						...
						'advanced_search_ap_title_font_size' => 'asea',
	...
	/**
	 * Adjust Main Page field specification when display it on Settings
	 *
	 * @param $field_specs
	 * @return array
	 */
	private function set_all_custom_field_specs( $field_specs ) {
		...
		switch ( $field_specs['name'] ) {
			...
			case 'template_main_page_display_title':
				$field_specs['dependency'] = ['templates_for_kb'];
				$field_specs['enable_on'] = ['kb_templates'];
				break;

			case 'template_main_page_display_title_info':
				$field_specs['dependency'] = ['templates_for_kb'];
				$field_specs['enable_on'] = ['current_theme_templates'];
				break;
	...
	private function any_of_requirements_missed( $requirements ) {
		...
		// a field might require an add-on plugins, layout, Main page, KB Templates enabled
		if ( ( ( in_array( 'not_elay', $requirements ) && $this->elay_enabled ) || ( in_array( 'elay', $requirements ) && ! $this->elay_enabled ) )
			...
			|| ( ( in_array( 'not_ai_search', $requirements ) && $this->ai_search_enabled ) || ( in_array( 'ai_search', $requirements ) && ! $this->ai_search_enabled ) )
			...
			|| ( ( in_array( 'not_block_main_page', $requirements ) && $this->kb_main_page_has_kb_blocks ) || ( in_array( 'only_block_main_page', $requirements ) && ! $this->kb_main_page_has_kb_blocks ) )
