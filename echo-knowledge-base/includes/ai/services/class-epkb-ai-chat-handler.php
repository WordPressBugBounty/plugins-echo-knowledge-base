...
class EPKB_AI_Chat_Handler extends EPKB_AI_Base_Handler {
	...
	public function process_message( $user_message, $conversation_obj, $collection_id ) {
		...
		$conversation_history = $conversation_obj->get_messages();
		...
		$save_result = $this->save_messages( $conversation_obj, $user_message, $ai_response, $assistant_message_id );
	 ...
	 */
	private function save_messages( $conversation_obj, $user_message, $ai_response, $assistant_message_id ) {
		...
		$user_message_id = 'msg_user_' . time() . '_' . uniqid();
		$messages[] = array(
			'id' => $user_message_id,
			'role' => 'user',
